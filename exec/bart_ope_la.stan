...
transformed parameters {
  ...
  rho = Phi_approx(mu_p[3] + sigma[3] * rho_p) * 2;
...
model {
  ...
  for (j in 1:N) {
    ...
    for (k in 1:Tsubj[j]) {
      real p_burst = 1 - ((phi[j] + eta[j] * n_succ) / (1 + eta[j] * n_pump));
      real u_gain = pow(reward[j, k], rho[j]);
      real u_loss;
      real u_pump;
      real u_stop = 0;
      real delta_u;
      ...
      for (l in 1:(pumps[j, k] + 1 - explosion[j, k])) {
        ...
        u_loss = pow((l - 1) * reward[j, k], rho[j]);
...
generated quantities {
  ...
  real<lower=0> mu_rho = Phi_approx(mu_p[3]) * 2;
    ...
    for (j in 1:N) {
      ...
      for (k in 1:Tsubj[j]) {
        ...
        for (l in 1:(pumps[j, k] + 1 - explosion[j, k])) {
          ...
          log_lik += bernoulli_logit_lpmf(d[j, k, l] | tau[j] * delta_u);

...
#'
#' @description
#' Hierarchical Bayesian Modeling of the Risky Decision Task (Rutledge et al., 2014, PNAS) with the following parameters: "rho" (risk aversion), "lambda" (loss aversion), and "tau" (inverse temp).
#'
#' \strong{MODEL:}
#' Prospect Theory (Sokol-Hessner et al., 2009, PNAS)
#'
#' @param data A .txt file containing the data to be modeled. Data columns should be labelled as follows: "subjID", "gain", "loss", "cert", "type", and "gamble". See \bold{Details} below for more information.
...
#' \describe{
#'  \item{\code{model}}{Character string with the name of the model (\code{"rdt_prospect"}).}
...
#' represent variables. For the Risky Decision Task, there should be six columns of data  with the labels
#' "subjID", "gain", "loss", "cert", "type", and "gamble". It is not necessary for the columns to be in this
...
#' output <- rdt_prospect(data = "example", niter = 2000, nwarmup = 1000, nchain = 3, ncore = 3)
#'
...
rdt_prospect <- function(data           = "choose",
                        niter          = 4000,
                        nwarmup        = 1000,
                        nchain         = 4,
                        ncore          = 1,

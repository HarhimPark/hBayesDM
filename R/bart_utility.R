...
bart_utility <- function(data           = "choose",
                            niter          = 4000,
                            nwarmup        = 1000,
                            nchain         = 4,
                            ncore          = 1,
  ...
  numPars <- 5
  POI     <- c("mu_phi", "mu_eta", "mu_rho", "mu_tau", "mu_lambda",
               "sigma",
  ...
  if (inits[1] != "random") {
    if (inits[1] == "fixed") {
      inits_fixed <- c(0.5, 1.0, 1.0, 1.0, 1.0)
    } else {
